...
class ProcessingTest(unittest.TestCase):
    ...
    def check_terms_index(self,response:Mapping,expected_response:Mapping):
        ...
        self.assertEqual(len(set_faltando),0,f"Os seguintes termos deveriam ter sido retornados: {set_faltando}")
        self.assertEqual(len(set_sobrando),0,f"Os seguintes termos não deveriam ter sido retornados: {set_sobrando}")
    ...
    def check_terms_occur(self,response:Mapping,expected_response:Mapping):
        ...
        if len(response.values())==0:
            self.assertEqual(len(expected_response.values()),0,"O retorno da função deveria ser um dicionário vazio")
            return
        if type(list(response.values())[0]) != list:
            map_resp_list = {term:[occur] for term,occur in response.items()}
            map_expected_resp_list = {term:[occur] for term,occur in expected_response.items()}
        else:
    ...
    def test_get_query_term_occurence(self):
        ...
        for i,expected_response in enumerate(arr_expected_response):
            ...
            response = self.queryRunner.get_query_term_occurence(arr_queries[i])
    ...
    def test_get_occurence_list_per_term(self):
        ...
        for i,expected_response in enumerate(arr_expected_response):
            ...
            response = self.queryRunner.get_occurrence_list_per_term(arr_terms[i])
