...
class RankingModelTest(unittest.TestCase):
    ...
    def test_boolean_model(self):
        ...
        arr_set_esperado_or_per_query  = [[{1,2,4},set()],[{1,2,3}]] # tinha um 3 que nao estava correto
        ...
        for idx, map_index in enumerate(self.arr_indexes):
            for query_position, map_query in enumerate(self.arr_queries_per_idx[idx]):
                ...
                self.assertSetEqual(set_response, arr_set_esperado_and_per_query[idx][query_position], msg=f"Consulta com operador AND obteve um resultado inesperado ({set_response}) para o indice {idx} consulta {query_position}. Esperava-se: {arr_set_esperado_and_per_query[idx][query_position]} ")

                model_or = BooleanRankingModel(OPERATOR.OR)
                lst_response,_ = model_or.get_ordered_docs(map_query, map_index_for_query)
                set_response =  set(lst_response)
                self.assertSetEqual(set_response, arr_set_esperado_or_per_query[idx][query_position], msg=f"Consulta com operador OR obteve um resultado inesperado ({set_response}) para o indice {idx} consulta {query_position}. Esperava-se: {arr_set_esperado_or_per_query[idx][query_position]} ")
